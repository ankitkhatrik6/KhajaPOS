...
# 1) Sync the current application source into the package tree. The dest is
...
rsync -a --delete \
    ...
    --exclude='node_modules' --exclude='demo' --exclude='tests' \
    --exclude='storage' --exclude='desktop' --exclude='packaging' \
    --exclude='.github' --exclude='run-web.sh' --exclude='stop-web.sh' \
    "$ROOT"/ "$TREE/opt/khajapos/"

# 2) The packaged app keeps its own empty env template; postinst fills in the
#    APP_KEY and DB_PASSWORD on the target machine during first install.
cp "$TREE/etc/khajapos/.env" "$TREE/opt/khajapos/.env.package"

# 3) Icons shipped by the package
...
# 4) Keep the empty runtime directories in the package
...
# 5) Never ship python bytecode caches or root-specific launcher scripts
...
# 6) Remove the previously built package so a broken build is obvious
rm -f "$DESKTOP"/khajapos_"${VERSION}"-1_all.deb

# 7) Build the package (file ownership is normalised to root)
